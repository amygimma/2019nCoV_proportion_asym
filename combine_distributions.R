...
) %>% gather(key = "distribution_name", "interval_days")


# Plot all
ggplot(serial_dists, aes(x = interval_days)) +
  geom_density() +
  ggtitle("Serial Distribution Density - all data") +
  xlab("Days") + ylab("Density")

# Plot by distribution
ggplot(serial_dists, aes(x = interval_days), color = "steelblue54") +
  geom_density() +
...
ggplot(early_serial, aes(x = interval_days)) +
...
fit_gamma <- fitdistrplus::fitdist(early_serial$interval_days, "gamma",
                                   optim.method = "Nelder-Mead")
fit_weibull <- fitdistrplus::fitdist(early_serial$interval_days, "weibull",
                                     optim.method = "Nelder-Mead")
...
by(fitted_serials$days,fitted_serials$distribution, summary)
...
write.csv(weibull_dist, file.path("outputs", "weibull_fitted_early_serial_dists.csv"))
write.csv(gamma_dist, file.path("outputs", "gamma_fitted_early_serial_dists.csv"))
